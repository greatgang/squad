...
class Batch(object):
    ...
    def __init__(self, context_ids, context_mask, context_tokens, qn_ids, qn_mask, qn_tokens, ans_span, ans_tokens, feature_ids, uuids=None):
        """
        Inputs:
...
def refill_batches(batches, word2id, context_file, qn_file, ans_file, feature_file, batch_size, context_len, question_len, discard_long):
    """
    ...
    tic = time.time()
    examples = [] # list of (qn_ids, context_ids, ans_span, ans_tokens, feature_ids) triples
    context_line, qn_line, ans_line, feature_line = context_file.readline(), qn_file.readline(), ans_file.readline(), feature_file.readline() # read the next line from each

    while context_line and qn_line and ans_line and feature_line: # while you haven't reached the end
        ...
        context_line, qn_line, ans_line, feature_line = context_file.readline(), qn_file.readline(), ans_file.readline(), feature_file.readline()
    ...
    for batch_start in xrange(0, len(examples), batch_size):
        ...
        context_ids_batch, context_tokens_batch, qn_ids_batch, qn_tokens_batch, ans_span_batch, ans_tokens_batch, feature_ids_batch = zip(*examples[batch_start:batch_start+batch_size])

        batches.append((context_ids_batch, context_tokens_batch, qn_ids_batch, qn_tokens_batch, ans_span_batch, ans_tokens_batch, feature_ids_batch))
...
def get_batch_generator(word2id, context_path, qn_path, ans_path, feature_path, batch_size, context_len, question_len, discard_long):
    """
    ...
    Inputs:
      ...
      discard_long: If True, discard any examples that are longer than context_len or question_len.
        If False, truncate those exmaples instead.
    """
    context_file, qn_file, ans_file, feature_file = open(context_path), open(qn_path), open(ans_path), open(feature_path)
    batches = []

    while True:
        if len(batches) == 0: # add more batches
            refill_batches(batches, word2id, context_file, qn_file, ans_file, feature_file, batch_size, context_len, question_len, discard_long)
        if len(batches) == 0:
            break

        # Get next batch. These are all lists length batch_size
        (context_ids, context_tokens, qn_ids, qn_tokens, ans_span, ans_tokens, feature_ids) = batches.pop(0)
        ...
        batch = Batch(context_ids, context_mask, context_tokens, qn_ids, qn_mask, qn_tokens, ans_span, ans_tokens, feature_ids)
